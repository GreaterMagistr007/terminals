...
### Что деплой **не** делает и делать не нужно

- **Ручной `git pull`** — не требуется, `deploy.sh` уже делает pull внутри (шаг 4). Запускать только сам `./deploy.sh`.
- **`php artisan queue:restart`** — не требуется. В проекте нет фоновых воркеров (`queue:work` / `queue:listen` не запущены под supervisor/systemd, в cron только `schedule:run`). Отправка в Telegram и прочие «тяжёлые» побочные эффекты происходят синхронно в HTTP-запросе или из `schedule:run`, свежий код подхватывается автоматически.
- **Перезапуск PHP-FPM / сброс opcache** — в штатном режиме не требуется. На хосте (`ispmanager`) `opcache.validate_timestamps=1`, FPM-процессы видят изменённые `.php` по mtime. Если после деплоя код почему-то не подхватился (сторонняя конфигурация opcache или ручные правки `.php` минуя git) — сбросить через панель ispmanager: **PHP → версия 8.3 → «Перезагрузить»**, либо у сайта `termials.in-site.ru` в разделе «Сайты» кнопка «Перезагрузить». Прав `sudo systemctl reload php-fpm` у пользователя `insite` нет.
